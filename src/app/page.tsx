...
export default function Page() {
  ...
  const signIn = async () => {
    ...
    const { error } = await supabase.auth.signInWithOtp({
      email,
      options: { emailRedirectTo: (process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000') + '/catalog' }
    });
  ...
  if (session) {
    return (
      <div className="card">
        <h2>Signed in</h2>
        <p className="small">You can browse the <a href="/catalog">Catalog</a> and place orders.</p>
